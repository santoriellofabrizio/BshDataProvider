...
class Handler(ABC):
    ...
    def handle(self, requests: List[Any], session: blpapi.Session, service: blpapi.Service) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            session: Bloomberg session instance
            service: Bloomberg service instance for API access
        ...
        if can_process:
            ...
            # Process in batch
            raw_out = self.process(can_process, session, service) or {}
        ...
        if downstream_requests and self._next:
            down = self._next.handle(downstream_requests, session, service)
...
class BulkFieldHandler(Handler, ABC):
    ...
    Bulk handlers process large datasets (dividend history, etc).
    """
...
class DailyPriceHandler(Handler, ABC):
    """
    Abstract base class for daily price handlers.

    Daily handlers process end-of-day market data.
    """

    def set_next(self, handler: 'DailyPriceHandler') -> 'DailyPriceHandler':
        """Link the next handler in the chain."""
        self._next = handler
        return handler
