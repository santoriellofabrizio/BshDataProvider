...
def cache_bsh_data(func=None, *, provider=None, category=None, readable_path=USE_READABLE_PATHS):
    ...
    Args:
        provider: Nome provider (auto-detect se None)
        category: Categoria cache (auto-detect se None)
        readable_path: Usa path leggibili invece di hash

    Usage:
        @cache_bsh_data
        def fetch_data(...):  # Auto-detect provider/category
            pass

        @cache_bsh_data(provider="timescale", category="market")
        def fetch_market_data(...):  # Explicit
            pass
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # Parametri da ignorare (non contribuiscono alla cache key)
        ignore_list = [
    ...
    # Handle decorator chiamato con o senza parentesi
    if func is None:
        return decorator
    else:
