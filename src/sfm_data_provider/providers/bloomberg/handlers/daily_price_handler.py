...
class BloombergDailyPriceHandler(DailyPriceHandler, BaseFetcher):
    ...
    def process(
        ...
        corr_ids = []
        for r in requests:
            bbg_root = getattr(sample.instrument, "root", None)
            suffix = getattr(sample.instrument, "suffix", None)
            sub = (r.subscription(bbg_root=bbg_root, current_date=r.end, suffix=suffix) if callable(r.subscription)
