...
class QueryOracle:
    ...
    def get_etf_fx(
        ...
        # Gestione ISIN list
        placeholders = self.conn.get_multiple_placeholders(3)
        params = {}

        params[placeholders[0]] = isin_list
        params[placeholders[1]] = day.strftime("%d-%m-%Y") if not isinstance(day, str) else day
        ...
        if currency:
            query += f" AND CURRENCY = {placeholders[2]}"
            params[placeholders[2]] = currency

        data, cols = self.conn.execute_query_many_placeholders(query, params)
    ...
    def get_etf_static_field(
        ...
        placeholders = self.conn.get_multiple_placeholders(1)
        params = {}
        ...
        query = f"""
            SELECT {cols_str}
            FROM AF_DATAMART_DBA.ETPS_INSTRUMENTS
            WHERE ISIN IN {placeholders[0]}
            ORDER BY ISIN
        """

        rows, cols = self.conn.execute_query_many_placeholders(query, params)
