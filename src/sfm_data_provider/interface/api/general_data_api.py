...
from sfm_data_provider.core.enums.fields import StaticField, MarketField
...
class GeneralDataAPI(BaseAPI):
    ...
    def get_bond_isins(
            self,
            classe: Optional[Union[Tuple, List]] = None,
            cache_provenienza: Optional[str] = "EURONEXT"
    ):
        """Lista ISIN ETP attivi."""

        return self.get(["bond_isins"],
                        classe=classe,
                        cache_provenienza=cache_provenienza,
                        source='timescale').get("BOND_ISINS")
