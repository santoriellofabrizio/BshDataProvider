...
from sfm_data_provider.core.instruments.classifier.instrument_classifier import InstrumentClassifier
...
class InstrumentFactory(Singleton):
    ...
    def _build_cdx(self, id_, ticker, autocomplete, **kwargs) -> CDXIndexInstrument:
        ...
        if autocomplete:
            for f in ("CURRENCY", "INDEX_NAME", "TENOR", "SERIES"):
                key = f.lower()
                cdx_kwargs[key] = kwargs.get(key) or self.classifier.get_cdx_field(ticker_root, f)

            if not self.classifier.cds.matches(ticker_root):
                logger.warning(f"{ticker_root} is not a recognized CDS root.")

        return cls(type=InstrumentType.CDXINDEX, ticker=id_, ticker_root=ticker_root, **cdx_kwargs)
    ...
    def _build_fx_forward(self, id, ticker, autocomplete, **param):
        ...
        if autocomplete:
            tenor = tenor or self.classifier.fx_forward.get_tenor(id)
