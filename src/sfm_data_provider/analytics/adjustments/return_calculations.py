...
class ReturnCalculator:
    ...
    def accumulate_returns_forward(self, returns: pd.DataFrame) -> pd.DataFrame:
        """
        Accumulate returns from each point to the end (forward-looking).

        Args:
            returns: DataFrame(dates × instruments) with returns

        Returns:
            DataFrame(dates × instruments) with forward cumulative returns

        Formula:
            - Percentage: p_T / p_t - 1 = reverse_cumprod(1 + r) - 1
            - Logarithmic: reverse_cumsum(r)
            - Absolute: reverse_cumsum(r)
