...
Typical usage:
    >>> from analytics.pipeline import EtfDataPipeline
    >>> 
    ...
    ... )
    >>> 
    >>> # Load all data (scarica tutto automaticamente)
    >>> pipeline.load()
    >>> 
    >>> # Access results
...
class EtfDataPipeline:
    ...
    Autosufficiente: scarica tutto automaticamente da BshData API.
    Permette override di componenti specifiche prima del load().
    ...
    def __init__(
            self,
            api,  # BshData instance
            instruments: Union[List[str], List],  # List of IDs or Instrument objects
        ...
        """
        Initialize ETF Data Pipeline.
        
        Args:
            api: BshData API instance
            instruments: List of ETF IDs or Instrument objects
            start: Start date
            end: End date
    ...
    def override_ter(self, ter: dict) -> "EtfDataPipeline":
        ...
        Args:
            ter: Dict {instrument_id: ter_value} (es. {"IE00B4L5Y983": 0.002})
    ...
    def override_fx_composition(self, fx_composition: pd.DataFrame) -> "EtfDataPipeline":
        ...
        Args:
            fx_composition: DataFrame with FX exposures
    ...
    def override_fx_forward_composition(self, fx_forward_composition: pd.DataFrame) -> "EtfDataPipeline":
        ...
        Args:
            fx_forward_composition: DataFrame with FX forward exposures
                                   Index: instrument IDs
    ...
    def override_fx_prices(self, fx_prices: pd.DataFrame) -> "EtfDataPipeline":
        ...
        Args:
            fx_prices: DataFrame with FX prices
                      Index: dates
                      Columns: currency pairs (EURUSD, EURGBP, etc.)
    ...
    def override_fx_forward_prices(self, fx_forward_prices: pd.DataFrame) -> "EtfDataPipeline":
        """
        Override FX forward prices.
        
        Args:
            fx_forward_prices: DataFrame with FX forward prices
    ...
    def disable_adjustment(self, *adjustments: str) -> "EtfDataPipeline":
        ...
        Args:
            *adjustments: Names of adjustments to disable
                         ('ter', 'fx_spot', 'fx_forward', 'dividends')
    ...
    def load(self) -> "EtfDataPipeline":
        ...
        # 2. Load FX composition and prices
    ...
    def _load_etf_prices(self):
        """Load ETF prices based on frequency."""
        logger.debug("Loading ETF prices...")
    ...
    def _load_fx_data(self):
        """Load FX composition and prices."""
        # FX Composition: use override or load from API
        if self._override_fx_composition is not None:
            self._fx_composition = self._override_fx_composition
        else:
            self._fx_composition = self._fetch_fx_composition()

        # FX Forward Composition: use override or load from API
        if self._override_fx_forward_composition is not None:
            self._fx_forward_composition = self._override_fx_forward_composition
        else:
            self._fx_forward_composition = self._fetch_fx_forward_composition()
        ...
        if self.config.adjust_fx_spot and currencies_spot:
            if self._override_fx_prices is not None:
                self._fx_prices = self._override_fx_prices
            else:
                self._fx_prices = self._fetch_fx_prices(currencies_spot)

        # FX Forward Prices
        if self.config.adjust_fx_forward and currencies_fwd:
            if self._override_fx_forward_prices is not None:
                self._fx_forward_prices = self._override_fx_forward_prices
            else:
                self._fx_forward_prices = self._fetch_fx_forward_prices(currencies_fwd)

    def _fetch_fx_composition(self) -> Optional[pd.DataFrame]:
        """Fetch FX composition from API."""
        try:
            # Assuming api.info has a method to get currency exposures
            currency_data = self.api.info.get_currency_exposure(id=self.instrument_ids)
            if currency_data is not None:
                # Pivot to: index=instrument_id, columns=currency, values=weight
                composition = currency_data.pivot(
                    index="index",
                    columns="CURRENCY",
                    values="WEIGHT"
                ).fillna(0)
                logger.info(f"Fetched FX composition: {composition.shape}")
                return composition
        except Exception as e:
            logger.warning(f"Could not fetch FX composition: {e}")
        return None

    def _fetch_fx_forward_composition(self) -> Optional[pd.DataFrame]:
        """Fetch FX forward composition from API."""
        try:
            currency_data = self.api.info.get_currency_exposure(id=self.instrument_ids)
            if currency_data is not None:
                composition = currency_data.pivot(
                    index="index",
                    columns="CURRENCY",
                    values="WEIGHT_FX_FORWARD"
                ).fillna(0)
                logger.info(f"Fetched FX forward composition: {composition.shape}")
                return composition
        except Exception as e:
            logger.warning(f"Could not fetch FX forward composition: {e}")
        return None

    def _get_currencies_from_composition(self, composition: Optional[pd.DataFrame]) -> List[str]:
        """Extract currency list from composition DataFrame."""
        if composition is None:
            return []
        currencies = composition.columns.tolist()
        # Remove EUR (base currency)
        return [c for c in currencies if c != "EUR"]

    def _fetch_fx_prices(self, currencies: List[str]) -> Optional[pd.DataFrame]:
        """Fetch FX spot prices from API."""
        if not currencies:
            return None
    ...
    def _fetch_fx_forward_prices(self, currencies: List[str]) -> Optional[pd.DataFrame]:
        """Fetch FX forward prices from API."""
        if not currencies:
            return None
    ...
    def _load_dividends(self):
        ...
        if self._override_dividends is not None:
            ...
            logger.debug("Loading dividends...")
    ...
    def _build_adjuster(self):
        """Build the Adjuster with all components."""
        logger.debug("Building Adjuster...")
        ...
        if self.config.adjust_dividends and self._dividends:
    ...
    def clean_prices(self) -> pd.DataFrame:
        """Clean prices (reconstructed from clean returns)."""
        self._ensure_loaded()
        return self._adjuster.clean_prices()
    ...
    def get_returns(
        ...
        Args:
            instrument_id: If provided, return Series for single instrument
            clean: If True, return clean returns; if False, raw returns
            
        Returns:
            DataFrame (all instruments) or Series (single instrument)
        """
    ...
    def resample(self, freq: str) -> pd.DataFrame:
        """
        Resample clean returns to different frequency.
        
        Args:
            freq: Target frequency ('5min', '15min', '1h', '1d', etc.)
