...
class CDSClassifier(BaseClassifier):
    ...
    def matches(self, identifier: str) -> bool:
        ...
        return idu in df.index or idu in df.get("INDEX_NAME").values.tolist() or "CDX" in identifier

    def get_field(self, ticker_root: str, field: str):
        df = self._load()
        ticker_root = ticker_root.upper()
        field = field.upper()
        
        ticker_root = ticker_root.replace('ITRAXXMAIN', 'ITXEB5')
        ticker_root = ticker_root.replace('ITRAXXXOVER', 'ITXEX5')
        ticker_root = ticker_root.replace('ITRAXXSUBFIN', 'ITXEU5')
        ticker_root = ticker_root.replace('ITRAXXSNRFIN', 'ITXES5')

        try:
            # Cerca tra gli indici quelli che iniziano con ticker_root (es. CDXIG -> CDXIG5)
            matches = df.index[df.index.str.startswith(ticker_root)]
            if not matches.empty:
                return df.at[matches[0], field]
            return None
        except Exception:
            return None
