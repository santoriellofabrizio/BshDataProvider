...
from sfm_datalibrary.connections.db_connections import DbConnectionParameters, \
...
from sfm_data_provider.core.requests.requests import BaseMarketRequest, GeneralRequest, DailyRequest, BaseStaticRequest
...
class TimescaleProvider(BaseProvider):
    ...
    def fetch_info_data(self, request: BaseStaticRequest | List[BaseStaticRequest]):
        if isinstance(request, List):
            if all(isinstance(r, GeneralRequest) for r in request):
                return self.fetcher.fetch_general_data(request)
