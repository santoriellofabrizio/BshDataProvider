...
def test_info_get_oracle_basic(api: BshData):
    ...
    fields = ["FUND_CURRENCY", "ETP_TYPE", "TER"]
...
def test_more_than_1000_get_oracle_fx_composition(api: BshData):
    print("\n========== TEST ORACLE FX COMPOSITION ==========")

    ids = api.general.get_etp_isins(segments=["IM"])
    df = api.info.get_fx_composition(isin=ids, fx_fxfwrd='fx')
